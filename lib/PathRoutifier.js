 ...
 *  * @param {Array.<Function>} middlewares all available middlewares
 *  * @param {MiddlewareStack} middlewareStack helper instance representing all current middlewares
 *  *   for this route.
 *  * @returns {Function|Array.<Function>} a set of express-compatible route callback(s) (e.g.
 *  *   function(req, res, next) { ... })
 *  *\/
 * module.exports = function(app, middlewares, middlewareStack) {
 * 		// Optionally reduce the current middleware stack
 * 		// middlewareStack.remove(middlewares.jsonBody)
 *
 ...
 * }
 *
 * Note: the middlewares argument is not available in global middlewares.
 *
 *
 * # Logging
		...
		dirTools.traverseDirectory(middlewaresPath, (listing) => {
			...
			.forEach((middlewareFileName) => {
				...
				if (this.autoNameAnonymousMiddleware_ && !ref[middlewareName].name)
					Object.defineProperty(ref[middlewareName], 'name', {value: middlewareName})
			})
		})
