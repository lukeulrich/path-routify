...
describe('PathRoutifier', function() {
	...
	describe('routify', function() {
		...
		it('handler module does not export anything throws error', function() {
			let app = express(),
				x = new PathRoutifier(app)

			expect(function() {
				x.routify(path.resolve(kTestRootRoutingErrorPath, 'no-function-exports'))
			}).throw(Error)
		})

		it('handler module with array export (non function) throws error', function() {
			let app = express(),
				x = new PathRoutifier(app)

			expect(function() {
				x.routify(path.resolve(kTestRootRoutingErrorPath, 'invalid-export'))
			}).throw(Error)
		})

		it('handler module returns invalid value (not a function or array) throws error', function() {
			let app = express(),
				x = new PathRoutifier(app)

			expect(function() {
				x.routify(path.resolve(kTestRootRoutingErrorPath, 'invalid-return'))
			}).throw(Error)
		})
		...
		describe('routes', function() {
			...
			function post(...params) {
				return test('post', ...params)
			}
			function put(...params) {
				return test('put', ...params)
			}
			function del(...params) {
				return test('delete', ...params)
			}
			function patch(...params) {
				return test('patch', ...params)
			}
